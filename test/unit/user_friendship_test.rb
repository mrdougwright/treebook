...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  context "#send_request_email" do
    setup do
      @user_friendship = UserFriendship.create user: users(:doug), friend: users(:mike)
    end
  ...
  context "#mutual_friendship" do
    setup do
      UserFriendship.request users(:doug), users(:jane)
      @friendship1 = users(:doug).user_friendships.where(friend_id: users(:jane).id).first
      @friendship2 = users(:jane).user_friendships.where(friend_id: users(:doug).id).first
    end
  ...
  context "@accept!" do
    setup do
      @user_friendship = UserFriendship.request users(:doug), users(:mike)
    end
  ...
  context ".request" do
    should "create two user friendships" do
      assert_difference "UserFriendship.count", 2 do
        UserFriendship.request(users(:doug), users(:mike))
      end
    end
  ...
  context "#block!" do
    ...
    should "not allow new requests once blocked" do
      @user_friendship.block!
      uf = UserFriendship.request users(:doug), users(:mike)
      assert !uf.save
    end
  end
end
